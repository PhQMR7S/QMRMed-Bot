(function () {
  ...
  var state = { route: 'home', me: null, plans: null };
  ...
  function bind() {
    ...
    document.querySelector('.brand').onclick = function () { go('home'); };
  }
  ...
  function plans() {
    var plans = state.plans || [
      { code:'FREE', name:'Free', subtitle:'البداية الأساسية مع تجربة QMRMed.', features:['تجربة المنصة التعليمية','المحتوى المجاني','الحساب والأرشيف الشخصي'] },
      { code:'PLUS', name:'PLUS', subtitle:'ميزات تعليمية وحدود AI موسعة.', features:['حدود AI أعلى','ميزات تعليمية موسعة','أرشيف موسع'] },
      { code:'PRO', name:'PRO', subtitle:'أعلى مستوى من المنصة.', features:['أولوية أعلى للذكاء الاصطناعي','ميزات متقدمة','أعلى حدود للاستخدام'] }
    ];
    view.innerHTML = '<div class="section-label">خطط الاشتراك</div>' + plans.map(function (p) {
      return '<section class="plan"><div class="plan-head"><div class="plan-icon">' + icon(p.code === 'FREE' ? 'medical' : 'crown') + '</div><div><h2>' + esc(p.name) + '</h2></div></div><div class="sub">' + esc(p.subtitle || '') + '</div><ul>' + (p.features || []).map(function (f) { return '<li>' + esc(f) + '</li>'; }).join('') + '</ul><button class="button" data-url="https://t.me/QMRMed">اختيار ' + esc(p.name) + '</button></section>';
    }).join('') + '<div class="notice">' + icon('info') + ' الدفع وتفعيل الاشتراك يتمان بأمان من داخل QMRMed Bot.</div>';
  }

  function account() {
    var m = state.me || { firstName:'QMRMed User', plan:'FREE' }, p = m.progress || {};
    view.innerHTML = '<div class="section-label">حسابي</div><section class="hero"><div class="profile"><div class="avatar">Q</div><div><h1>' + esc([m.firstName,m.lastName].filter(Boolean).join(' ') || 'QMRMed User') + '</h1><p>' + esc(m.username ? '@' + m.username : 'Telegram user') + '</p><div class="plan-pill">' + icon('crown') + ' ' + esc(m.plan || 'FREE') + '</div></div></div></section><div class="list"><div class="row"><span>القسم</span><strong>' + esc(m.department || 'غير محدد') + '</strong></div><div class="row"><span>المرحلة</span><strong>' + esc(m.stage || 'غير محددة') + '</strong></div><div class="row"><span>الدروس المكتملة</span><strong>' + (p.completed || 0) + '</strong></div><div class="row"><span>المحاولات</span><strong>' + (p.attempts || 0) + '</strong></div><div class="row"><span>الدقة</span><strong>' + (p.accuracy || 0) + '%</strong></div></div>';
  }
  ...
  function render() {
    ...
    else if (state.route === 'archive') view.innerHTML = '<div class="section-label">الأرشيف</div><div class="empty">' + icon('archive') + '<br>لا توجد نتائج محفوظة حتى الآن.</div>';
    bind();
  }
  ...
  function loadUser() {
    api('/api/me').then(function (data) {
      ...
      state.me = { firstName:'بك', plan:'FREE', archiveCount:0, progress:{ completed:0, attempts:0, accuracy:0 } };
      render();
      if (toast) {
        toast.textContent = error.message || 'بيانات Telegram غير متاحة';
        toast.classList.add('show');
      }
    });
    api('/api/plans').then(function (data) { state.plans = data; if (state.route === 'plans') render(); }).catch(function () {});
  }
