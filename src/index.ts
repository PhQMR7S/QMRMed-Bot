...
bot.command('plans', async ctx => ctx.reply('💎 اشتراكات QMRMed\n\n🆓 FREE — المحتوى الأساسي\n💙 PLUS — مزايا موسعة\n💜 PRO — الوصول الكامل\n\nسيتم تفعيل الاشتراك فقط بعد التحقق من الدفع.', { reply_markup: new InlineKeyboard().text('💙 PLUS', 'plan:PLUS').text('💜 PRO', 'plan:PRO').row().text('📜 الشروط', 'terms').row().text('⬅️ الرئيسية', 'home') }));
...
bot.command('terms', async ctx => ctx.reply('📜 شروط الاستخدام\n\nاستخدام QMRMed تعليمي فقط. المحتوى لا يُعد تشخيصًا أو استشارة طبية شخصية. قبل شراء أي اشتراك، راجع شروط الاستخدام وسياسة الدعم الخاصة بالدفع.', { reply_markup: backMenu }));
bot.command('paysupport', async ctx => ctx.reply('💳 دعم الدفع\n\nلأي مشكلة تتعلق بالدفع أو الاشتراك، أرسل تفاصيل العملية إلى دعم QMRMed. لا ترسل مفاتيح أو رموزًا سرية.', { reply_markup: backMenu }));
...
bot.callbackQuery('plans', async ctx => { await ctx.answerCallbackQuery(); await render(ctx, '💎 اشتراكات QMRMed\n\n🆓 FREE — المحتوى الأساسي\n💙 PLUS — مزايا موسعة\n💜 PRO — الوصول الكامل\n\nسيتم تفعيل الاشتراك فقط بعد التحقق من الدفع.', new InlineKeyboard().text('💙 PLUS', 'plan:PLUS').text('💜 PRO', 'plan:PRO').row().text('📜 الشروط', 'terms').row().text('⬅️ الرئيسية', 'home')); });
bot.callbackQuery(/^plan:(PLUS|PRO)$/, async ctx => { await ctx.answerCallbackQuery(); await render(ctx, `💎 ${ctx.match[1]}\n\nالزر لا يمنح الاشتراك مباشرة. عند تفعيل الدفع، يجب أن تتم عملية الشراء عبر Telegram Stars ثم التحقق من successful_payment قبل منح الخطة.`, new InlineKeyboard().text('📜 الشروط', 'terms').row().text('💳 دعم الدفع', 'paysupport').row().text('⬅️ الاشتراك', 'plans')); });
...
bot.on('message:text', async ctx => {
  ...
  try { const section = /\bcase\b|\bcase\s+study\b|حالة سريرية|كيس/i.test(text) ? 'cases' : 'study'; const response = await answerMedicalQuestion(text, { department: user.department ?? undefined, stage: user.stage ?? undefined, section, plan: user.plan }); return replyLong(ctx, `🤖 QMRMed AI\n\n${response}`); } catch (error) { console.error('AI failed:', error); return ctx.reply('⚠️ تعذر الوصول إلى المساعد الذكي حاليًا. تأكد من إعداد OmniRoute وقاعدة البيانات.'); }
});
