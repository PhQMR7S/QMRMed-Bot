...
async function replyLong(ctx: Context, text: string, replyMarkup?: InlineKeyboard) {
  ...
  for (let i = 0; i < chunks.length; i++) await ctx.reply(chunks[i], i === chunks.length - 1 && replyMarkup ? { reply_markup: replyMarkup } : undefined);
}
...
async function finishQuiz(ctx: Context, key: string, timedOut = false) {
  const session = sessions.get(key); sessions.delete(key); if (!session) return;
...
async function showAccount(ctx: Context) {
  const user = await getUser(ctx);
  await ctx.editMessageText(`👤 حسابي\n\n🆔 Telegram ID: ${user.telegramId}\n👤 الاسم: ${[user.firstName, user.lastName].filter(Boolean).join(' ') || '-'}\n🎯 القسم: ${user.department ?? 'غير محدد'}\n📚 المرحلة: ${user.stage ?? 'غير محددة'}\n💎 الخطة: ${user.plan}\n🎁 التجربة: ${user.trialUsed ? 'مستخدمة' : 'متاحة'}`, { reply_markup: new InlineKeyboard().text('🎯 تغيير القسم/المرحلة', 'study_mode').row().text('⬅️ الرئيسية', 'home') });
}
async function showCommands(ctx: Context) {
  await ctx.reply('📖 أوامر QMRMed\n\n/start — الرئيسية\n/help — المساعدة\n/study — وضع الدراسة\n/search — البحث داخل المحتوى المعتمد\n/ai — المساعد الذكي\n/questions — بنك الأسئلة\n/ministerial — الوزاريات\n/exams — الاختبارات\n/progress — تقدمي\n/plans — الاشتراك\n/trial — التجربة\n/account — حسابي\n/settings — الإعدادات\n/about — عن QMRMed\n/cancel — إلغاء العملية الحالية\n/admin — لوحة الإدارة للمشرف فقط');
}
...
bot.command('help', async (ctx) => showCommands(ctx));
bot.command('study', async (ctx) => { await getUser(ctx); await ctx.reply('🎯 اختر القسم الدراسي:', { reply_markup: studyDepartmentMenu() }); });
...
bot.command('progress', async (ctx) => { const user = await getUser(ctx); const [completed, attempts, correct] = await Promise.all([db.progress.count({ where: { userId: user.id, completed: true } }), db.attempt.count({ where: { userId: user.id } }), db.attempt.count({ where: { userId: user.id, correct: true } })]); const accuracy = attempts ? Math.round((correct / attempts) * 100) : 0; await ctx.reply(`📊 تقدمي\n\nالدروس المكتملة: ${completed}\nالمحاولات: ${attempts}\nالإجابات الصحيحة: ${correct}\nالدقة: ${accuracy}%`, { reply_markup: backMenu }); });
bot.command('plans', async (ctx) => ctx.reply('💎 اشتراكات QMRMed\n\n🆓 FREE — المحتوى الأساسي\n💙 PLUS — مزايا دراسية موسعة\n💜 PRO — الوصول الكامل\n\nسيتم ربط الدفع الفعلي عبر Telegram Stars قبل الإنتاج.', { reply_markup: new InlineKeyboard().text('💙 PLUS', 'plan:PLUS').text('💜 PRO', 'plan:PRO').row().text('⬅️ الرئيسية', 'home') }));
bot.command('trial', async (ctx) => { const user = await getUser(ctx); if (user.trialUsed) return ctx.reply('🎁 تم استخدام التجربة المجانية لهذا الحساب بالفعل.', { reply_markup: backMenu }); const updated = await ensureTrial(user.id); await ctx.reply(`🎁 تم تفعيل التجربة المجانية.\nتنتهي في: ${updated.trialEndsAt?.toLocaleString('ar-IQ') ?? '-'}`, { reply_markup: backMenu }); });
bot.command('account', async (ctx) => { await showAccount(ctx); });
bot.command('settings', async (ctx) => ctx.reply('⚙️ الإعدادات\n\n🎯 استخدم /study لتحديد القسم والمرحلة.', { reply_markup: backMenu }));
bot.command('about', async (ctx) => ctx.reply('ℹ️ QMRMed\n\nمنصة تعليمية لطلاب المجموعة الطبية، تجمع المحتوى الدراسي، بنك الأسئلة، الوزاريات، الاختبارات، التقدم والمساعد الذكي.', { reply_markup: backMenu }));
...
bot.callbackQuery(/^subject:(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const subject = await db.subject.findUnique({ where: { id: Number(ctx.match[1]) }, include: { topics: { orderBy: { order: 'asc' } } } }); if (!subject) return ctx.editMessageText('المادة غير موجودة.', { reply_markup: backMenu }); await ctx.editMessageText(`📖 ${subject.name}\n\n${subject.description ?? 'اختر الموضوع:'}`, { reply_markup: topicMenu(subject.topics, subject.id) }); });
bot.callbackQuery(/^topic:(\d+):(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const topic = await db.topic.findUnique({ where: { id: Number(ctx.match[1]) }, include: { lessons: { orderBy: { order: 'asc' } } } }); if (!topic) return ctx.editMessageText('الموضوع غير موجود.', { reply_markup: backMenu }); await ctx.editMessageText(`📘 ${topic.name}\n\nاختر الدرس:`, { reply_markup: lessonMenu(topic.lessons, topic.id, Number(ctx.match[2])) }); });
bot.callbackQuery(/^topicback:(\d+):(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const subject = await db.subject.findUnique({ where: { id: Number(ctx.match[2]) }, include: { topics: { orderBy: { order: 'asc' } } } }); if (!subject) return ctx.editMessageText('المادة غير موجودة.', { reply_markup: backMenu }); await ctx.editMessageText(`📖 ${subject.name}\n\nاختر الموضوع:`, { reply_markup: topicMenu(subject.topics, subject.id) }); });
bot.callbackQuery(/^lesson:(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const lesson = await db.lesson.findUnique({ where: { id: Number(ctx.match[1]) }, include: { topic: true } }); if (!lesson) return ctx.editMessageText('الدرس غير موجود.', { reply_markup: backMenu }); const user = await getUser(ctx); const progress = await db.progress.upsert({ where: { userId_lessonId: { userId: user.id, lessonId: lesson.id } }, create: { userId: user.id, lessonId: lesson.id, completed: false }, update: {} }); const keyboard = new InlineKeyboard().text(progress.completed ? '✅ مكتمل' : '☑️ تعليم كمكتمل', `complete:${lesson.id}`).row().text('⬅️ الموضوع', `topicback:${lesson.topicId}:${lesson.topic.subjectId}`).row().text('🏠 الرئيسية', 'home'); await replyLong(ctx, `📖 ${lesson.title}\n\n${lesson.content}\n\n${lesson.source ? `📚 المصدر: ${lesson.source}` : ''}`, keyboard); });
...
bot.callbackQuery(/^ans:(\d+):(.+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const key = userKey(ctx); const session = sessions.get(key); if (!session) return ctx.reply('انتهت الجلسة. ابدأ تدريبًا جديدًا من القائمة.'); if (isQuizExpired(session, Date.now())) return finishQuiz(ctx, key, true); const questionId = Number(ctx.match[1]); const selected = ctx.match[2]; if (session.ids[session.index] !== questionId) return ctx.reply('⚠️ هذا السؤال لم يعد فعالًا. استخدم آخر سؤال ظاهر أمامك.'); const question = await db.question.findUnique({ where: { id: questionId } }); if (!question || !canUseInButtonQuiz(question)) return ctx.reply('⚠️ تعذر معالجة هذا السؤال.'); const selectedValue = resolveSelectedValue(question, selected); const correct = isAnswerCorrect(question, selected); const user = await getUser(ctx); if (correct) session.correct++; session.answered++; session.index++; await db.attempt.create({ data: { userId: user.id, questionId: question.id, answer: selectedValue, correct } }); await ctx.editMessageReplyMarkup({ reply_markup: new InlineKeyboard().text(correct ? '✅ صحيح' : `❌ خطأ — الصحيح: ${question.answer}`, 'quiz:noop') }); if (session.index >= session.ids.length) return finishQuiz(ctx, key); await showQuestion(ctx, key); });
...
bot.callbackQuery('progress', async (ctx) => { await ctx.answerCallbackQuery(); const user = await getUser(ctx); const [completed, attempts, correct] = await Promise.all([db.progress.count({ where: { userId: user.id, completed: true } }), db.attempt.count({ where: { userId: user.id } }), db.attempt.count({ where: { userId: user.id, correct: true } })]); const accuracy = attempts ? Math.round((correct / attempts) * 100) : 0; await ctx.editMessageText(`📊 تقدمي\n\nالدروس المكتملة: ${completed}\nالمحاولات: ${attempts}\nالإجابات الصحيحة: ${correct}\nالدقة: ${accuracy}%`, { reply_markup: backMenu }); });
bot.callbackQuery('study_mode', async (ctx) => { await ctx.answerCallbackQuery(); await ctx.editMessageText('🎯 اختر القسم الدراسي:', { reply_markup: studyDepartmentMenu() }); });
bot.callbackQuery(/^study:department:(Medicine|Dentistry|Pharmacy)$/, async (ctx) => { await ctx.answerCallbackQuery(); await ctx.editMessageText(`🎯 ${ctx.match[1]}\n\nاختر المرحلة:`, { reply_markup: studyStageMenu(ctx.match[1]) }); });
bot.callbackQuery(/^study:stage:(Medicine|Dentistry|Pharmacy):(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery('تم حفظ التفضيل'); const user = await getUser(ctx); await db.user.update({ where: { id: user.id }, data: { department: ctx.match[1], stage: ctx.match[2] } }); await ctx.editMessageText(`✅ تم حفظ وضع الدراسة\n\nالقسم: ${ctx.match[1]}\nالمرحلة: ${ctx.match[2]}`, { reply_markup: new InlineKeyboard().text('📚 الدراسة', 'subjects').row().text('🏠 الرئيسية', 'home') }); });
...
bot.callbackQuery(/^admin(?::.*)?$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!adminGuard(ctx)) return ctx.editMessageText('غير مصرح.'); const data = ctx.callbackQuery.data; try {
  if (data === 'admin:stats') { const d = await getAdminDashboard(); return ctx.editMessageText(`📊 Dashboard QMRMed\n\n👥 المستخدمون: ${d.users}\n🆓 FREE: ${d.free}\n💙 PLUS: ${d.plus}\n💜 PRO: ${d.pro}\n\n📚 المواد: ${d.subjects}\n📘 المواضيع: ${d.topics}\n📖 الدروس: ${d.lessons}\n❓ بنك الأسئلة: ${d.questions}\n📝 الوزاريات: ${d.ministerialQuestions}\n🎯 المحاولات: ${d.attempts}\n\n☁️ المصادر المعتمدة: ${d.approvedSources}\n🔎 المصادر المفهرسة: ${d.indexedSources}\n🧩 المقاطع: ${d.chunks}`, { reply_markup: adminMenu() }); }
  if (data === 'admin:users') { const d = await getAdminDashboard(); return ctx.editMessageText(`👥 المستخدمون\n\nإجمالي: ${d.users}\nFREE: ${d.free}\nPLUS: ${d.plus}\nPRO: ${d.pro}`, { reply_markup: adminMenu() }); }
  if (data === 'admin:content') { const d = await getAdminDashboard(); const breakdown = await getContentBreakdown(); const labels: Record<string, string> = { SOURCE: '📚 Sources', REFERENCE: '📖 References', QUESTION: '❓ Question Bank', MINISTERIAL: '📝 Ministerial', CASE: '🩺 Cases' }; const kinds = breakdown.map((item) => `${labels[item.kind] ?? item.kind}: ${item.count}`).join('\n'); return ctx.editMessageText(`📚 المحتوى\n\n${kinds}\n\nالمواد: ${d.subjects}\nالمواضيع: ${d.topics}\nالدروس: ${d.lessons}\nالأسئلة المنظمة: ${d.questions}\nالوزاريات المنظمة: ${d.ministerialQuestions}\n\nالمصادر المعتمدة: ${d.approvedSources}\nالمصادر المفهرسة: ${d.indexedSources}\nالمقاطع المفهرسة: ${d.chunks}`, { reply_markup: adminMenu() }); }
  if (data === 'admin:questions') { const d = await getAdminDashboard(); return ctx.editMessageText(`📝 الأسئلة والوزاريات\n\n❓ بنك الأسئلة: ${d.questions}\n📝 الوزاريات: ${d.ministerialQuestions}\n🎯 المحاولات: ${d.attempts}\n\nالوصول الفعلي للأسئلة يخضع لخطة المستخدم/التجربة كما هو معرف في طبقة الصلاحيات.`, { reply_markup: adminMenu() }); }
  if (data === 'admin:drive') { const status = await getDriveStatus(); if (!status.configured) return ctx.editMessageText('☁️ Google Drive\n\n❌ غير مُهيأ.\n\nأضف GOOGLE_DRIVE_ROOT_FOLDER_ID وبيانات Service Account إلى بيئة تشغيل البوت.', { reply_markup: adminDriveMenu() }); const state = status.state; return ctx.editMessageText(`☁️ Google Drive\n\nالحالة: مُهيأ\nRoot Folder ID: ${status.rootFolderId}\n\nآخر تشغيل: ${state?.lastRunAt?.toLocaleString('ar-IQ') ?? '-'}\nآخر نجاح: ${state?.lastSuccessAt?.toLocaleString('ar-IQ') ?? '-'}\nالملفات المقروءة: ${state?.filesSeen ?? 0}\nالملفات المفهرسة في آخر تشغيل: ${state?.filesIndexed ?? 0}\n${state?.lastError ? `❌ آخر خطأ: ${state.lastError}` : '✅ لا يوجد خطأ مسجل'}`, { reply_markup: adminDriveMenu() }); }
  if (data === 'admin:drive:status') { const status = await getDriveStatus(); if (!status.configured) return ctx.editMessageText('☁️ Google Drive غير مُهيأ حاليًا.', { reply_markup: adminDriveMenu() }); const state = status.state; return ctx.editMessageText(`☁️ حالة Google Drive\n\nRoot: ${status.rootFolderId}\nآخر تشغيل: ${state?.lastRunAt?.toLocaleString('ar-IQ') ?? '-'}\nآخر نجاح: ${state?.lastSuccessAt?.toLocaleString('ar-IQ') ?? '-'}\nملفات: ${state?.filesSeen ?? 0}\nمفهرسة: ${state?.filesIndexed ?? 0}\n${state?.lastError ? `❌ ${state.lastError}` : '✅ آخر تشغيل ناجح/بدون خطأ مسجل'}`, { reply_markup: adminDriveMenu() }); }
  if (data === 'admin:drive:sync') { const status = await getDriveStatus(); if (!status.configured) return ctx.editMessageText('❌ لا يمكن بدء المزامنة: Google Drive غير مُهيأ.', { reply_markup: adminDriveMenu() }); await ctx.editMessageText('🔄 جاري مزامنة Google Drive وفهرسة المحتوى…'); const result = await runAdminDriveSync(); return ctx.editMessageText(`✅ اكتملت مزامنة Google Drive\n\n📄 الملفات المقروءة: ${result.filesSeen}\n🔎 الملفات المفهرسة/المحدثة: ${result.filesIndexed}\n⏭️ الملفات المتخطاة: ${result.filesSkipped}\n🗑️ الملفات التي أزيل فهرسها: ${result.filesRemoved}\n🧩 المقاطع الجديدة: ${result.chunks}`, { reply_markup: adminDriveMenu() }); }
  if (data === 'admin:broadcast') { pending.set(userKey(ctx), 'broadcast'); return ctx.editMessageText('📣 أرسل الآن رسالة الإرسال الجماعي. سيتم إرسالها للمستخدمين المسجلين.', { reply_markup: backMenu }); }
  if (data === 'admin:ai') { const result = await omniChat([{ role: 'user', content: 'Reply only: QMRMed OmniRoute OK' }], { temperature: 0 }); return ctx.editMessageText(`🤖 OmniRoute يعمل.\n\n${result}`, { reply_markup: adminMenu() }); }
  return ctx.editMessageText('🛠️ لوحة الإدارة', { reply_markup: adminMenu() });
} catch (error) { console.error('Admin action failed:', error); return ctx.editMessageText(`❌ تعذر تنفيذ العملية.\n\n${error instanceof Error ? error.message : String(error)}`, { reply_markup: data.startsWith('admin:drive') ? adminDriveMenu() : adminMenu() }); } });
...
bot.on('message:text', async (ctx) => { const user = await getUser(ctx); const key = userKey(ctx); const text = ctx.message.text.trim(); if (text.startsWith('/')) return; const action = pending.get(key); if (!action) return ctx.reply('استخدم /start لفتح لوحة QMRMed.', { reply_markup: mainMenu }); pending.delete(key);
  if (action === 'broadcast') { if (!isAdmin(ctx.from.id)) return ctx.reply('غير مصرح.'); const users = await db.user.findMany({ select: { telegramId: true } }); let sent = 0; for (const target of users) { try { await ctx.api.sendMessage(target.telegramId, `📣 QMRMed\n\n${text}`); sent++; } catch { /* Continue after blocked/deleted chats. */ } } return ctx.reply(`✅ اكتمل الإرسال. نجح: ${sent}/${users.length}`); }
  if (action === 'search') { try { const results = await searchApprovedContent(text, { take: 10 }); if (!results.length) return ctx.reply('لم أجد نتيجة مطابقة داخل محتوى QMRMed المعتمد.'); const context = formatRetrievedContext(results, 10_000); return replyLong(ctx, `🔎 نتائج البحث داخل QMRMed المعتمد:\n\n${context}`); } catch (error) { console.error(error); return ctx.reply('⚠️ تعذر البحث في محتوى QMRMed حاليًا. تأكد من تشغيل مزامنة Google Drive وقاعدة البيانات ثم حاول مرة أخرى.'); } }
  try { const response = await answerMedicalQuestion(text, ''); return replyLong(ctx, `🤖 QMRMed AI\n\n${response}`); } catch (error) { console.error(error); return ctx.reply('⚠️ تعذر الوصول إلى المساعد الذكي حاليًا. تأكد من إعداد OmniRoute ثم حاول مرة أخرى.'); }
});
bot.catch((err) => console.error('QMRMed Bot error:', err.error));
process.once('SIGINT', () => bot.stop());
process.once('SIGTERM', () => bot.stop());
bot.start({ onStart: async (info) => { console.log(`@${info.username} is running`); await bot.api.setMyCommands([{ command: 'start', description: 'فتح QMRMed' }, { command: 'help', description: 'الأوامر والمساعدة' }, { command: 'study', description: 'القسم والمرحلة' }, { command: 'search', description: 'البحث في محتوى QMRMed' }, { command: 'ai', description: 'المساعد الذكي' }, { command: 'questions', description: 'بنك الأسئلة' }, { command: 'ministerial', description: 'الأسئلة الوزارية' }, { command: 'exams', description: 'الاختبارات' }, { command: 'progress', description: 'التقدم والنتائج' }, { command: 'plans', description: 'الاشتراكات' }, { command: 'trial', description: 'التجربة المجانية' }, { command: 'account', description: 'حسابي' }, { command: 'settings', description: 'الإعدادات' }, { command: 'about', description: 'عن QMRMed' }, { command: 'cancel', description: 'إلغاء العملية الحالية' }, { command: 'admin', description: 'لوحة الإدارة' }]); } });
