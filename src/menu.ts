...
export function studyDepartmentMenu() {
  return new InlineKeyboard().text('🩺 طب عام', 'study:department:Medicine').row().text('🦷 طب أسنان', 'study:department:Dentistry').row().text('💊 صيدلة', 'study:department:Pharmacy').row().text('🏠 الرئيسية', 'home');
}
export function studyStageMenu(department: string) {
  return new InlineKeyboard().text('1️⃣ المرحلة 1', `study:stage:${department}:1`).text('2️⃣ المرحلة 2', `study:stage:${department}:2`).row().text('3️⃣ المرحلة 3', `study:stage:${department}:3`).text('4️⃣ المرحلة 4', `study:stage:${department}:4`).row().text('5️⃣ المرحلة 5', `study:stage:${department}:5`).text('6️⃣ المرحلة 6', `study:stage:${department}:6`).row().text('⬅️ الأقسام', 'study_mode').row().text('🏠 الرئيسية', 'home');
}
export function plansMenu() {
  return new InlineKeyboard().text('💙 PLUS — شهر | 250⭐', 'plan:PLUS:30').row().text('💜 PRO — شهر | 500⭐', 'plan:PRO:30').row().text('🔥 PLUS — 5 أشهر | 500⭐', 'plan:PLUS:150').row().text('🔥 PRO — 5 أشهر | 1000⭐', 'plan:PRO:150').row().text('🏆 PLUS — سنة | 1000⭐', 'plan:PLUS:365').row().text('🏆 PRO — سنة | 2000⭐', 'plan:PRO:365').row().text('💳 الاشتراك والتفعيل الخارجي', 'payment_methods').row().text('🎟️ تفعيل كود اشتراك', 'redeem_code').row().text('📜 الشروط', 'terms').text('🆘 دعم الدفع', 'paysupport').row().text('⬅️ الرئيسية', 'home');
}
export function externalPaymentMenu() {
  return new InlineKeyboard().text('💙 PLUS — شهر | 5$', 'manual:PLUS:30').row().text('💜 PRO — شهر | 10$', 'manual:PRO:30').row().text('🔥 PLUS — 5 أشهر | 10$', 'manual:PLUS:150').row().text('🔥 PRO — 5 أشهر | 20$', 'manual:PRO:150').row().text('🏆 PLUS — سنة | 20$', 'manual:PLUS:365').row().text('🏆 PRO — سنة | 40$', 'manual:PRO:365').row().text('🎟️ لدي كود تفعيل', 'redeem_code').row().text('⬅️ الاشتراكات', 'plans').row().text('🏠 الرئيسية', 'home');
}
