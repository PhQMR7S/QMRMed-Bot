...
/**
 * Canonical QMRMed Drive taxonomy.
 * Database values intentionally remain stable English identifiers so the
 * existing bot/user study filters continue to work, while Drive itself
 * remains Arabic/canonical.
 */
const DEPARTMENTS: Record<string, string> = {
  ...
  'المختبرات': 'Medical Laboratory Techniques',
  ...
  'الطوارئ': 'Emergency Medical Techniques',
  ...
  'عناية قلب': 'Cardiac Care Techniques',
...
const MATERIALS_FOLDER_NAMES = new Set([
  'materials',
  'material',
  'المواد',
  'مادة',
...
export function normalizeFolderName(part: string) {
  return part.trim().replace(/^\d+\s*-\s*/, '').trim();
}

export function normalizedSegments(path: string) {
  return path.split('/').filter(Boolean).map(normalizeFolderName);
}
...
function isMaterialsFolder(part: string) {
  const normalized = normalizeFolderName(part).toLocaleLowerCase();
  return MATERIALS_FOLDER_NAMES.has(normalized);
}
...
export function contentKind(path: string): ContentKind | null {
  ...
  // In the canonical hierarchy, normal study material lives under:
  // Department / Stage / 01 - المواد / Canonical Subject / file
...
function canonicalDepartment(part: string) {
  const normalized = normalizeFolderName(part).toLocaleLowerCase();
  return DEPARTMENTS[normalized] ?? null;
}

/**
 * Extract department/stage/subject from the final Arabic Drive hierarchy.
 *
 * Supported canonical forms include:
 *   Department / Stage / 01 - المواد / Subject / file
 *   Department / Stage / 02 - المصادر والمراجع / Subject / file
 *   Department / Stage / 03 - الأسئلة الوزارية / Subject / file
 *   Department / Stage / 04 - أسئلة إضافية / Subject / file
 *   Department / Stage / 05 - حالات سريرية / Subject / file
 *
 * For stage-level content without a subject folder, subjectName remains null.
 */
export function metadataFromPath(path: string) {
  ...
  if (stageIndex >= 0) {
    ...
    if (kindIndex >= 0) {
      ...
      if (afterKind.length > 0) subjectName = afterKind[0];
    }
  }
...
export async function syncGoogleDrive() {
  ...
  // Normalize a pasted Drive URL before using it as the persistent sync key.
  ...
  try {
    ...
    if (!files.length) {
      throw new Error('Google Drive scan returned zero files; stale-content cleanup was intentionally skipped. Verify root folder and credentials before retrying.');
    }
    ...
    for (const file of files) {
      ...
      // Link to an existing canonical Subject when one exists. Search filters
      // remain driven by ContentSource department/stage/subjectName, so a
      // missing legacy Subject row can never make Drive content disappear.
      const subject = meta.subjectName
        ? await db.subject.findFirst({
            where: { name: meta.subjectName },
            select: { id: true },
          })
        : null;
    ...
    // Never deindex everything merely because the Drive scan found no
    // indexable files. This guard protects the knowledge base from an
    // accidentally wrong root or temporarily inaccessible hierarchy.
